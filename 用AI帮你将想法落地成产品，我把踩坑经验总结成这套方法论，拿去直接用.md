...
输入 /superpowers:brainstorm 聊需求，出需求文档
  ↓
学习竞品：让AI搜竞品或者现成的优先产品
  ↓
界面设计要好看：
给awesome-design规范做设计
  ↓
分解任务：
...
刚才提到的 `/superpowers:brainstorm`、`/superpowers:write-plan`、`/superpowers:execute-plan`，都来自同一个工具：**superpowers**。

**Superpowers 是什么？**

Superpowers 是一套 AI 工作流指令集，本质上就是三段文字。你把这三段文字分别粘贴给任何 AI 工具（OpenClaw、Claude Code、Cursor、Trae 都可以），AI 就会按照这套方法论帮你想清楚需求、拆解任务、分步执行。

不是软件，不用安装，复制粘贴就能用。

**Superpowers 能做什么？**
- `/superpowers:brainstorm` — AI 像产品经理一样追问你的需求，帮你把想法梳理清楚
- `/superpowers:write-plan` — 把确认好的需求拆成一个个可执行的小步骤
- `/superpowers:execute-plan` — 按步骤一步步执行，每步让你确认再做下一步
...
**4. Claude Code**
地址：https://claude.ai/claude-code
作用：Anthropic官方CLI编程工具
用法：命令行操作，适合开发者
...
作用：AI编程工具，支持手机IM下发命令
用法：可在微信等IM里下命令，人不在电脑前也能跑

**7. awesome-design**
地址：https://github.com/VoltAgent/awesome-design-md
作用：AI设计规范文档，复制粘贴给AI即可
用法：复制全文+设计需求发给AI，AI按规范执行

**8. Superpowers**
地址：https://github.com/obra/superpowers
作用：AI工作流指令集，三段文字复制粘贴
用法：见上文「什么是 superpowers」章节
